...
The chat agent answers free text questions about the NG12 guidelines using RAG. Unlike the Part 1 agent (which uses tool calling), the chat agent follows a simpler pipeline: retrieve chunks, inject them as context into the prompt and have the LLM synthesize an answer with citations.
...
2. **Synthesize, do not copy** — the prompt explicitly instructs the model to summarize and restructure retrieved passages in its own words. This was added to fix an issue where answers would just dump raw guideline text back to the user as in the raw json format.
...
1. **Exact match or similar match** against a set of known greeting patterns (e.g., "hi", "good morning", "thanks")
...
ChromaDB always returns results, even for non relevant queries. To prevent the LLM from hallucinating answers from irrelevant chunks, a **similarity threshold** (cosine distance > 1.2) gates the pipeline:

- If the best-matching chunk has a distance above 1.2, the system returns a "couldn't find relevant info" disclaimer
- The LLM is never called with weak eviden this prevents it from answering without context 
...
- If the answer is a disclaimer, any citations the LLM may have attached are discarded
- This runs on both successfully parsed JSON responses and fallback raw-text responses

### Citation Enforcement

If the LLM returns a clinical answer but forgets to include citations (and good evidence was retrieved), the system automatically derives citations from the top 3 retrieved chunks.
...
The last 20 messages (approximately 10 user-assistant exchanges) are injected into the LLM prompt as prior conversation turns. This keeps the context window manageable while supporting multi-turn follow-ups
...
- No re-embedding per chat request

## Temperature

Set to **0.1** — same as Part 1. Clinical guideline Q&A requires consistent, reproducible answers. Low temperature minimizes creativity that could introduce inaccuracies.
